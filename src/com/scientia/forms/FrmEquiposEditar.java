 ...
 * @author danim
 */
public class FrmEquiposEditar extends javax.swing.JFrame {

    private Equipo objActualizar;
    private Map<Integer, Integer> losCodigosEquipo = new HashMap<>();

    public FrmEquiposEditar(JFrame parent, boolean modal, Equipo equipo) {
        initComponents();
        cargarDatosEquipo(equipo);
        this.objActualizar = equipo;
        //máximizar pantalla
    ...
    private void cargarDatosEquipo(Equipo equipo) {
        cajaInventario.setText(equipo.getCod_inventario_equipo());
        cajaModelo.setText(equipo.getModelo_equipo());
        cajaSerie.setText(equipo.getSerie_equipo());
        cmbMantenimiento.setSelectedItem(equipo.getMantenimiento_equipo());
        cmbReq.setSelectedItem(equipo.getReq_mante_equipo());
        cmbEstado.setSelectedItem(equipo.getEstado_equipo());
        cajaNombre.setText(equipo.getNombre_equipo());
        cajaMarca.setText(equipo.getMarca_equipo());
        chkDisponible.setSelected(equipo.getDisponibilidad_equipo());
        cajaExistencias.setText(String.valueOf(equipo.getExistencias_equipo()));
    }
    ...
    private void initComponents() {
        ...
        btnAnadir.setText("Editar Equipo");
        ...
        btnAnadir.setBounds(400, 580, 190, 30);
    ...
    private void btnAnadirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAnadirActionPerformed
        ...
        if (estaTodoBien()) {
            ...
            // Corregimos el mapeo de los códigos de equipo
            Integer idMantenimiento = losCodigosEquipo.get(mantenimiento);
            Integer idReqMante = losCodigosEquipo.get(reqMante);
            Integer idEstado = losCodigosEquipo.get(estado);

            // Corregimos la asignación del idEquipo, ya que se estaba sobrescribiendo
            idEquipo = idMantenimiento != null ? idMantenimiento : 0;
            idEquipo = idReqMante != null ? idReqMante : 0;
            idEquipo = idEstado != null ? idEstado : 0;

            objActualizar.setCod_inventario_equipo(codInventario);
            objActualizar.setModelo_equipo(modelo);
            objActualizar.setSerie_equipo(serie);
            objActualizar.setMantenimiento_equipo(mantenimiento);
            objActualizar.setReq_mante_equipo(reqMante);
            objActualizar.setEstado_equipo(estado);
            objActualizar.setNombre_equipo(nombre);
            objActualizar.setMarca_equipo(marca);
            objActualizar.setDisponibilidad_equipo(disponibilidad);
            objActualizar.setExistencias_equipo(existencias);

            DaoEquipo miDao = new DaoEquipo();
            if (miDao.actualizar(objActualizar)) {
                JOptionPane.showMessageDialog(jpContainer, "Se actualizó", "INFORMACIÓN", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(jpContainer, "Error al actualizar", "ERROR", JOptionPane.ERROR_MESSAGE);
            }
        }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                new FrmEquiposEditar(null, true, null).setVisible(true);
            }
        });
    }
